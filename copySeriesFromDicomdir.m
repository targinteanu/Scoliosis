function copySeriesFromDicomdir(src, patt, dst, sep_folders)
...
% Written 2010-05-25 by Jadrian Miles
	...
	for k=1:length(patient{i}.study{j}.series)
		...
		if regexpi(s.info.Modality, patt)
			...
			dstdir = fullfile(dst, strjoin({num2str(i),pname, num2str(j),stname, num2str(k),srname}, '.'));
			...
			for l=1:length(s.image)
                ...
                if sep_folders
                    ...
                    fdst = [dstdir, sprintf(' img%03d.dcm', l)];
                end
				copyfile(fsrc, fdst);
			end
			fprintf('Done.\n');
