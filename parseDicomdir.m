function [patient, modalities] = parseDicomdir(fname)
    ...
    for i=1:length(patient)
        for j=1:length(patient{i}.study)
            for k=1:length(patient{i}.study{j}.series)
                [patient{i}.study{j}.series{k}.info.Modality, ' | ', ...
                patient{i}.study{j}.series{k}.info.SeriesDescription, ' | ',...
                patient{i}.study{j}.series{k}.info.BodyPartExamined]
            end
        end
    end
    %}
    
    modalities = cell(1, length(patient)*length(patient{1}.study)*length(patient{1}.study{1}.series));
    idx = 1; 
    for i=1:length(patient)
        for j=1:length(patient{i}.study)
            for k=1:length(patient{i}.study{j}.series)
                modalities{idx} = patient{i}.study{j}.series{k}.info.Modality;
                idx=idx+1;
            end
        end
    end
